...
func registerTools(s *server.MCPServer, c *Client) {
	...
	s.AddTool(
		mcp.NewTool("post_content",
			...
			mcp.WithString("content",
				mcp.Required(),
				mcp.Description("Markdown body to store. Do not start with a duplicate H1 title; knowledged stores the title separately in YAML frontmatter."),
			),
			mcp.WithString("hint",
	...
	s.AddTool(
		mcp.NewTool("edit_content",
			...
			mcp.WithString("content",
				mcp.Required(),
				mcp.Description("Replacement Markdown body. Do not start with a duplicate H1 title; knowledged stores the title separately in YAML frontmatter."),
			),
			mcp.WithString("title",
